...
(function (Model) {
	...
	Object.assign(Model.Please3D.prototype, {
		...
		initSection: function () {
			...
			_this.clipPlaney2 = new THREE.Plane(new THREE.Vector3(0, -1, 0), vy + vspc);
			_this.clipPlanez1 = new THREE.Plane(new THREE.Vector3(0, 0, 1), vz + vspc);
			_this.clipPlanez2 = new THREE.Plane(new THREE.Vector3(0, 0, -1), vz + vspc);
			_this.clipPlanes = [_this.clipPlanex1, _this.clipPlanex2, _this.clipPlaney1, _this.clipPlaney2, _this.clipPlanez1, _this.clipPlanez2];
		},
		...
		clearSection: function () {
			if (null != this.clipSelection) {
				for (var i = 0; i < this.clipSelection.displayMeshes.children.length; i++) {
		...
		resetSection: function (bFlag) {
			...
			if (bFlag) {
				...
				if (_this.vecModelCenter) {
					...
					var vpx2 = 5;
					var tMax = new THREE.Vector3(_this.vecModelBox.max.x, _this.vecModelBox.max.y, _this.vecModelBox.max.z);
					var tMin = new THREE.Vector3(_this.vecModelBox.min.x, _this.vecModelBox.min.y, _this.vecModelBox.min.z);
